...
count(ship_entity.product_id) over (partition by ship_entity.ekl_vendor_tracking_id 
ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as count_of_shipment_items,
sum(ship_entity.item_quantity) over (partition by ship_entity.ekl_vendor_tracking_id 
ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as total_quantity,
ship_entity.profiler_flag
from 
(SELECT 
data.vendor_tracking_id AS ekl_vendor_tracking_id,
...
LATERAL VIEW explode(data.shipment_items) exploded_table AS shipment_item
group by data.vendor_tracking_id, shipment_item.product_id
) as ship_entity
